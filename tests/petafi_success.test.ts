...
describe('bitfi-sol-smartcontract', () => {
  ...
  describe('Deposit() and Claim with SOL successfully', () => {
    ...
    it(`Should claim successfully`, async () => {
      ...
      const afterVaultBalance = await connection.getBalance(vaultPda, { commitment: 'confirmed' });
      assert.equal(beforeVaultBalance - afterVaultBalance, amount * LAMPORTS_PER_SOL, 'Vault balance should decrease by the amount of SOL deposited');
      const userTradeDetailData = await getTradeDetailData(tradeId, connection);
      const status = userTradeDetailData.status;
  ...
  describe('Deposit(), setFee and settlement, withdraw fee with SOL successfully', () => {
    ...
    it('Should withdraw total fee successfully', async () => {
      ...
      assert.equal(beforeProtocolBalance - afterProtocolBalance, feeAmount * LAMPORTS_PER_SOL, 'The protocol balance should decrease by the amount of fee');
      assert.equal(afterFeeReceiverBalance - beforeFeeReceiverBalance, feeAmount * LAMPORTS_PER_SOL, 'The fee receiver balance should increase by the amount of fee');
    })
  });
  ...
  describe('Deposit(), setFee and settle successfully with WSOL', () => {
    ...
    it('Should withdraw total fee successfully', async () => {
      ...
      assert.equal(beforeProtocolBalance - afterProtocolBalance, feeAmount * 10**9, 'The protocol balance should decrease by the amount of fee');
      assert.equal(afterFeeReceiverBalance - beforeFeeReceiverBalance, feeAmount * 10**9, 'The fee receiver balance should increase by the amount of fee');
    })
  });
});
