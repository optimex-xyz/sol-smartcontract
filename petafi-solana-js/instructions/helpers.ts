 ...
 */
export async function createAssociatedTokenAccountInstructionIfNeeded(connection: Connection, payer: PublicKey, tokenPubkey: PublicKey | null, userPubkey: PublicKey) {
    if (!tokenPubkey) return [];
    const userTokenAta = await getAssociatedTokenAddressSync(tokenPubkey, userPubkey, true);
    const userTokenAtaInfo = await connection.getAccountInfo(userTokenAta, 'confirmed');
    if (!userTokenAtaInfo) {
...
export async function verifyPresignSettlement(params: VerifyPresignSettlementParam): Promise<{ error: InvalidPresignStringError | null, isVerified: boolean }> {
    const { connection, tradeId, pmmPubkey, presign } = params;
    // MPC must validate the data of the asset chain and L2 to make sure the trade is valid
    const tradeDetailData = await getTradeDetailData(tradeId, connection);
    ...
    const nonceAccountInfo = await connection.getAccountInfo(tradeDetailData.userEphemeralPubkey);
 ...
 */
export async function getTokenBalance(connection: Connection, tokenPubkey: PublicKey | null, accountPubkey: PublicKey, commitment: Commitment = 'confirmed'): Promise<number> {
    if (!tokenPubkey) {
