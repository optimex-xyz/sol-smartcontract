...
import { getOffchainProgram as getOffchainpetafiProgram } from "../artifacts";
 ...
 */
const program = getOffchainpetafiProgram();
...
export type GetPaymentReceiptPdaParam = {
    ...
    toUser: PublicKey;
    /**
     * The amount of the payment, without decimals
     */
    amount: string;
    /**
     * The protocol fee of the payment, without decimals
     */
    protocolFee: string;
    /**
    ...
    /**
     * The decimals of the token, 9 for SOL native token
     */
...
export function getPaymentReceiptPda(paymentArgs: GetPaymentReceiptPdaParam) {
    ...
    const amountWithDecimals = new BN(Number(paymentArgs.amount) * 10 ** paymentArgs.tokenDecimals);
    const protocolFeeWithDecimals = new BN(Number(paymentArgs.protocolFee) * 10 ** paymentArgs.tokenDecimals);
    const [paymentReceiptPda] = PublicKey.findProgramAddressSync(
        [
            ...
            amountWithDecimals.toArrayLike(Buffer, 'le', 8),
            protocolFeeWithDecimals.toArrayLike(Buffer, 'le', 8),
 ...
 * @param programId - The programId, default is the petafi program
