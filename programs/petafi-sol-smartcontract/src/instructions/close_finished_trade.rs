...
use crate::{assert_keys_equal, error::PetaFiError, state::*};
use anchor_lang::prelude::*;
use anchor_spl::{associated_token::get_associated_token_address_with_program_id, token::{self, Token, TokenAccount}};
...
pub fn handler_close_finished_trade<'c: 'info, 'info>(
    ...
    user_trade_detail.assert_close_finished_trade(current_timestamp, config.close_trade_duration, is_mpc)?;

    match user_trade_detail.token {
        // If trade is with token, close the token account
        Some(token) => {
            if let Some(vault_token_account) = vault_token_account {
                let calculated_vault_ta = get_associated_token_address_with_program_id(&vault.key(), &token, &ctx.accounts.token_program.key);
                assert_keys_equal(&vault_token_account.key(), &calculated_vault_ta, PetaFiError::InvalidTokenAccount)?;
                // if vault_token_account.mint != token || vault_token_account.owner != vault.key() {
                //     return Err(PetaFiError::InvalidTokenAccount.into());
                // }
                let seeds: &[&[u8]] = &[b"vault", &close_finished_trade_args.trade_id, &[ctx.bumps.vault]];
                ...
                // Must pass a token account if the trade is with token
                return Err(PetaFiError::InvalidTokenAccount.into());
            }
        }
