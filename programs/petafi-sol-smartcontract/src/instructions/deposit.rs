...
use crate::{constants::*, error::PetaFiError, state::*, utils::*, ID};
...
pub fn handler_deposit<'c: 'info, 'info>(
    ...
    if !ephemeral_account.data_is_empty() {
        return Err(PetaFiError::NonceAccountBeingUsed.into());
    }
    ...
    assert_keys_equal(&user_key_input, signer.key, PetaFiError::Unauthorized)?;
...
pub struct DepositAccounts<'info> {
    ...
    #[account(
        owner = ID @ PetaFiError::NotWhitelistedToken,  // This PDA must come from our smart-contract
    )]
