...
use crate::{error::PetaFiError, event::*, state::*, utils::*, ID};
...
pub struct SettlementAccounts<'info> {
    ...
    #[account(
        mut,
        address = user_trade_detail.mpc_pubkey @ PetaFiError::Unauthorized // Check authorization
    )]
    ...
    #[account(
        mut,
        address =  user_trade_detail.user_pubkey @ PetaFiError::InvalidUserAccount, // check user account
    )]
    ...
    #[account(
        address = user_trade_detail.user_ephemeral_pubkey @ PetaFiError::Unauthorized, // Check user ephemeral pubkey
    )]
