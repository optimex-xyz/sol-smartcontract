...
use crate::{PetaFiError, TradeDetail, ID};
...
/// # Errors
/// * [PetaFiError::Unauthorized] when the caller is not authorized, or not the [TradeDetail::mpc_pubkey].
...
pub fn handler_set_total_fee(
    ...
    if current_timestamp > user_trade_detail.timeout {
        return err!(PetaFiError::TimeOut);
    }
