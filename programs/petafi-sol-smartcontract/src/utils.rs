...
pub fn assert_keys_equal(key1: &Pubkey, key2: &Pubkey, error_type: PetaFiError) -> Result<()> {
...
pub fn transfer_spl_token<'c: 'info, 'info>(
    ...
    assert_keys_equal(&mint.key(), &token_pubkey, PetaFiError::InvalidMintKey)?;
    ...
    assert_keys_equal(&source.key(), &source_key, PetaFiError::InvalidSourceAta)?;
    ...
    assert_keys_equal(
        &destination.key(),
        &destination_key,
        PetaFiError::InvalidDestinationAta,
    )?;
...
pub fn vec_u8_to_publickey(v: &Vec<u8>) -> Result<Pubkey> {
    let key_str = String::from_utf8(v.to_vec()).map_err(|_| PetaFiError::InvalidPublicKey)?;
    let result = Pubkey::from_str(&key_str).map_err(|_| PetaFiError::InvalidPublicKey)?;
    Ok(result)
}
...
pub fn vec_u8_to_address(v: &Vec<u8>) -> Result<Option<Pubkey>> {
    ...
    match key_str.as_str() {
        "native" => Ok(None),
        _ => Ok(Some(Pubkey::from_str(&key_str).map_err(|_| PetaFiError::InvalidPublicKey)?)),
    }
}
