...
impl TradeDetail {
    ...
    pub fn assert_claim(&self) -> Result<()> {
        ...
        if current_timestamp <= self.timeout {
            return Err(PetaFiError::ClaimNotAvailable.into());
        }
        if self.status != TradeStatus::Deposited {
            return Err(PetaFiError::InvalidTradeStatus.into());
        }
    ...
    pub fn assert_settlement(&self) -> Result<()> {
        ...
        let current_timestamp = clock.unix_timestamp;
        if current_timestamp > self.timeout {
            return Err(PetaFiError::TimeOut.into());
        }
        if self.status != TradeStatus::Deposited {
            return Err(PetaFiError::InvalidTradeStatus.into());
        }
            ...
            TradeStatus::Settled => {
                if is_mpc {
                    ...
                    if current_timestamp <= (self.timeout as u64) + close_wait_duration {
                        return Err(PetaFiError::CloseNotAvailable.into());
                    }
                }
            }
        }
...
pub fn test_assert_close_depsited_trade() {
    ...
    assert_eq!(result.unwrap_err(), PetaFiError::InvalidTradeStatus.into());
...
pub fn test_assert_close_settled_trade() {
    ...
    let result = trade_detail.assert_close_finished_trade(0, 0, false);
    assert_eq!(result.unwrap_err(), PetaFiError::CloseNotAvailable.into());
