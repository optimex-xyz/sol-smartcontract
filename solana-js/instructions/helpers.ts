...
import { Commitment, Connection, NonceAccount, PublicKey, Transaction } from '@solana/web3.js'
 ...
 */
export async function verifyPresignSettlement(
  params: VerifyPresignSettlementParam
): Promise<{ error: InvalidPresignStringError | null; isVerified: boolean }> {
  ...
  if (numberOfSignaturesPair !== 2) {
    const error = new InvalidPresignStringError('Invalid number of signatures', {
      expected: 2,
      actual: numberOfSignaturesPair,
    })
    return { error, isVerified: false }
  }
  ...
  if (mpcSignaturePair.publicKey.toBase58() !== tradeDetailData.mpcPubkey.toBase58()) {
    const error = new InvalidPresignStringError('Invalid MPC pubkey', {
      expected: tradeDetailData.mpcPubkey.toBase58(),
      actual: mpcSignaturePair.publicKey.toBase58(),
    })
    return { error, isVerified: false }
  }
  if (mpcSignaturePair.signature !== null) {
    const error = new InvalidPresignStringError(`MPC's presign signature is not null`, {
      expected: null,
      actual: Buffer.from(mpcSignaturePair.signature).toString('hex'),
    })
    return { error, isVerified: false }
  }
...
export async function getTokenBalance(
  ...
  try {
    ...
    throw new FetchAccountError('Failed to fetch token account balance', {
      tokenPubkey: tokenPubkey.toBase58(),
      accountPubkey: accountPubkey.toBase58(),
      ata: tokenAta.toBase58(),
    })
  }
}
