 ...
 */
export type GetTradeInputParams = Pick<
  DepositInstructionParam,
  'userPubkey' | 'toUserAddress' | 'solver' | 'amount' | 'sessionId' | 'connection'
> & {
  /**
 ...
 */
export async function getTradeInput(params: GetTradeInputParams) {
  ...
  const fromChainAsBuffer: Buffer[] = fromChain.map((u8Array) => Buffer.from(u8Array))

  const toChainAsBuffer: Buffer[] = toChain.map((u8Array) => Buffer.from(u8Array))
  ...
  const tradeId = getTradeId(sessionId, solver, {
